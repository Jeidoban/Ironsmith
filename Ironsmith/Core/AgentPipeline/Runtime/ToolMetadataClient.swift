...
struct ToolPromptRefinementClient: Sendable {
    ...
    nonisolated private static let additivePromptRefinementInstructions = """
        ...
        - If external setup is required, must ask the coding agent to include clear in-app setup instructions covering any service configuration, schema or SQL, permissions, and client-safe endpoints, API keys, or tokens. It must never request embedding server-side secrets.
        - Must not add an app-owned account system, iCloud, CloudKit, push notifications, analytics, subscriptions, or cross-device sync unless explicitly requested.
