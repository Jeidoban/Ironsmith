...
enum ToolGenerationPrompts {
    static let flameSingleFileCodingInstructions = """
        ...
        The generated app is a self-contained client-side macOS app. Do not assume or invent an undeployed backend, custom server, or placeholder server-dependent functionality. You may connect directly to an existing hosted API or managed service when it meaningfully supports the user's request and requires no custom backend deployment. If only a general-purpose backend is needed and the user did not name a specific service, prefer Supabase and direct the user to https://database.new to create their project. If external setup is required, include clear in-app instructions for creating and configuring the service, applying any required schema or SQL, granting permissions, and entering client-safe endpoints, API keys, or tokens. Never embed server-side secrets. Do not add iCloud/CloudKit integration, push notifications, analytics, subscriptions, or cross-device sync unless the user requests them.
